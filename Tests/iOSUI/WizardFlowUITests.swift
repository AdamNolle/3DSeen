...
final class WizardFlowUITests: XCTestCase {
    ...
    func testWizardRoutesThroughCaptureAndExposesReviewSurface() {
        ...
        app.buttons["Continue to Detail"].tap()
        assertScreen("quality")

        app.buttons.matching(NSPredicate(format: "label BEGINSWITH 'Capture at'")).firstMatch.tap()
        ...
        app.launch()
        assertScreen("review")
        XCTAssertTrue(app.staticTexts["Review & compute"].waitForExistence(timeout: 3))
    }
